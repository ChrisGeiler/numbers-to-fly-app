...
function generateFlySightConfig({
  ...
  const alarms = [
    { elev: alarm9, type: 4, file: "9" },
    { elev: alarm5, type: 4, file: "5" },
    { elev: alarm4, type: 4, file: "4" },
    { elev: alarm3, type: 4, file: "3" },
    { elev: alarm2, type: 4, file: "2" },
    { elev: alarm1, type: 4, file: "1" },
    { elev: alarmBeep, type: 1, file: "0" },
    { elev: alarmFlare, type: 4, file: "flare" },
    { elev: alarmTask, type: 4, file: taskLabel },
  ];

  const alarmText = alarms
    .filter((alarm) => alarm.elev.trim() !== "")
    .map(
      (alarm) => `Alarm_Elev:    ${Math.round(
        numberFromInput(alarm.elev, 0)
      )} ; Alarm elevation (m above ground level)
Alarm_Type:    ${alarm.type} ; Alarm type
Alarm_File:    ${alarm.file} ; File to be played`
    )
    .join("\n\n");

  return `; Generated by Numbers to Fly
; FlySight version: ${
    flySightVersion === "original" ? "Original FlySight" : "FlySight 2"
  }
; Task: ${taskLabel}
; Direct Bluetooth transfer is not included in this version.
...
; UNDER NO CIRCUMSTANCES SHOULD THESE ALARMS BE USED TO INDICATE DEPLOYMENT OR BREAKOFF ALTITUDE.
...
Win_Top:       4300
Win_Bottom:    ${Math.round(numberFromInput(alarmBeep, 2500))}
`;
}
...
export default function App() {
  ...
  const [configAlarm5, setConfigAlarm5] = useState("3040");
  const [configAlarm4, setConfigAlarm4] = useState("2940");
  const [configAlarm3, setConfigAlarm3] = useState("2840");
  const [configAlarm2, setConfigAlarm2] = useState("2740");
  const [configAlarm1, setConfigAlarm1] = useState("2640");
  const [configAlarmBeep, setConfigAlarmBeep] = useState("2510");
      ...
      <main className="app">
        ...
        <section className="card">
          ...
          <div className="config-actions">
            <button type="button" onClick={copyGeneratedConfig}>
              Copy config
            </button>

            <button type="button" onClick={downloadGeneratedConfig}>
              Download TXT
            </button>
          </div>
